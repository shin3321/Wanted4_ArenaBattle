...
AABItemBox::AABItemBox()
{
	...
	Mesh->SetCollisionProfileName(TEXT("NoColiision"));
...
void AABItemBox::OnOverlapBegin(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	// 아이템이 설정되어 있으면 캐릭터에 아이템 획득 ㅁ메시지 전달(인터페이스를 통해)
	...
	// 파티클 재생이 종료되면 제거하기 위해 델리게이트 등록
	Effect->OnSystemFinished.AddDynamic(this, &AABItemBox::InEffectFinished);
}

void AABItemBox::InEffectFinished(UParticleSystemComponent* PSystem)
{
	Destroy();
}
